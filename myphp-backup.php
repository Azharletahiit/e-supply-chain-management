...
class Backup_Database
{
    ...
    protected function initializeDatabase()
    {
        try {
            ...
            if (!mysqli_set_charset($conn, $this->charset)) {
                mysqli_query($con, $conn, 'SET NAMES ' . $this->charset);
            }
        } catch (Exception $e) {
    ...
    public function backupTables($tables = '*')
    {
        try {
            ...
            if ($tables == '*') {
                $tables = array();
                $result = mysqli_query($con, $this->conn, 'SHOW TABLES');
            ...
            foreach ($tables as $table) {
                ...
                $row = mysqli_fetch_row(mysqli_query($con, $this->conn, 'SHOW CREATE TABLE `' . $table . '`'));
                $sql .= "\n\n" . $row[1] . ";\n\n";
                ...
                $row = mysqli_fetch_row(mysqli_query($con, $this->conn, 'SELECT COUNT(*) FROM `' . $table . '`'));
                $numRows = $row[0];
                ...
                for ($b = 1; $b <= $numBatches; $b++) {
                    ...
                    $result = mysqli_query($con, $this->conn, $query);
